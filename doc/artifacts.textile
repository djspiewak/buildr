...
repositories.remote << 'http://www.ibiblio.org/maven2'      # add Ibilio's repo
{% endhighlight %}

Repositories will be scanned for artifacts in the order in which they appear in this array.  Thus, if you prefer Ibilio's repository, but you still have some artifacts which are only available from Maven.org, you can include both with Ibilio as the "first in line":
